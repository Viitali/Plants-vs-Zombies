...
public class Game {
    ...
    public boolean userWon() {
    	if (zManager.noZombiesLeft() && zombieList.noZombies()) {
    		...
    		{
    			Level l = level.nextLevel(level);
    			if(l==null)
    				return true;
    			else
    			{
    				zManager.setZombiesLeftToAppear(l.getNumberOfZombies());
    				zManager.setFrecuencia(l.getZombieFrequency());
    				return false;
    			}
    		}
    	}
    ...
    public void update() {
       plantList.update();
       zombieList.update();
       GameHistory hs = new GameHistory(plantList.clone(),zombieList.clone(),cycleCount,zManager.getZombiesLeftToAppear()
    		   ,sManager.getSunCoins(),level.copy());
       history.add(hs);
       idxhistory++;
    ...
    {
    	if(idxhistory>0)
    	{ // da error a veces pero la idea es esta
    		idxhistory--;;
    		this.plantList=history.get(idxhistory).getPlantsList();
        	this.zombieList=history.get(idxhistory).getZombieList();
        	this.cycleCount=history.get(idxhistory).getCycles();
        	this.zManager.setZombiesLeftToAppear(history.get(idxhistory).getRemainingZombies());
        	this.sManager.setSunCoins(history.get(idxhistory).getSuncoins());
        	this.level=history.get(idxhistory).getLevel();
    ...
    public boolean delete(int x, int y) throws CommandExecuteException
    {
    	if(plantList.isObjectInPosition(x, y))
    	{
    		plantList.kill(x,y);
    		sManager.addCoins(25);
    		return true;
    	}
    	else
    		throw new CommandExecuteException ("No hay ninguna planta en esa posicion");
    }
    ...
    {
    	return zombieList.freezeZombie(posX,posY);
    }
